...
async function collect(helpers) {
  ...
  if (homeText) {
    ...
    log.info(`kuaishou: home flat (${flat.length} chars): ${flat.substring(0, 500)}`);
    result.followers = findMetric(homeText, ["粉丝数", "粉丝总数", "粉丝", "关注者"], log);
    result.totalViews = findMetric(homeText, ["播放量", "总播放量", "播放", "展现量", "曝光"], log);
    result.totalLikes = findMetric(homeText, ["点赞数", "获赞", "点赞", "赞"], log);
    result.totalComments = findMetric(homeText, ["评论数", "评论量", "评论"], log);
    result.totalShares = findMetric(homeText, ["分享数", "转发数", "分享"], log);
    result.contentCount = findMetric(homeText, ["作品数", "视频数", "投稿数", "已发布"], log);
  }
  ...
  if (dataText) {
    ...
    log.info(`kuaishou: data flat (${flat.length} chars): ${flat.substring(0, 500)}`);
    if (result.followers === 0) result.followers = findMetric(dataText, ["粉丝数", "粉丝", "关注"], log);
    if (result.totalViews === 0) result.totalViews = findMetric(dataText, ["播放量", "总播放", "播放"], log);
    if (result.totalLikes === 0) result.totalLikes = findMetric(dataText, ["点赞数", "获赞", "点赞"], log);
